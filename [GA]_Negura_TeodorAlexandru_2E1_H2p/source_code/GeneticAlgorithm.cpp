...
void GeneticAlgorithm::mutatePolynomial(Individual& ind) {
    double eta_m_local = eta_m; // Use member variable
    std::uniform_real_distribution<double> probDist(0.0, 1.0);
    std::uniform_real_distribution<double> randDist(0.0, 1.0);
    ...
    for (int i = 0; i < dimension; ++i) {
        if (probDist(rng) < mutationRate) {
            ...
            double delta;
            if (u <= 0.5) {
                delta = std::pow(2.0 * u, 1.0 / (eta_m_local + 1.0)) - 1.0;
            } else {
                delta = 1.0 - std::pow(2.0 * (1.0 - u), 1.0 / (eta_m_local + 1.0));
            }
