...
#include "AntColonyOptimized.h"  // Use the optimized version
...
// Run optimized ACO on a single instance and save results to file
RunResult runExperiment(const InstanceConfig& config, int runNumber = 1, bool verbose = true, bool writeTour = true, bool writeFile = true) {
    ...
    // Create optimized ACO with self-tuning
    AntColonyOptimized aco(tsp);
    aco.setEnableEarlyTermination(true);
    aco.setTargetGap(1.0);  // Stricter target for better performance
    ...
    std::ofstream out;
    if (writeFile) {
        outputFile = "results/ACO_opt_" + config.name + "_run" + std::to_string(runNumber) + ".txt";
        out.open(outputFile);
        if (!out.is_open()) {
    ...
    if (writeFile) {
        out << "============================================" << std::endl;
        out << "OPTIMIZED ANT COLONY OPTIMIZATION RESULTS" << std::endl;
        out << "============================================" << std::endl;
    ...
    if (writeFile) {
        out << "--- OPTIMIZED ACO Parameters ---" << std::endl;
        out << "Number of Ants: " << aco.getNumAnts() << std::endl;
        out << "Max Iterations: " << (instance_size >= 1000 ? 3000 : 2000) << std::endl;
        out << "Alpha (Pheromone Importance): 0.8-1.0" << std::endl;
        out << "Beta (Heuristic Importance):   2.5-3.0" << std::endl;
        out << "Rho (Evaporation Rate):        0.01-0.02" << std::endl;
        out << "Local Search:                  2-opt improvement, MAX-MIN Ant System" << std::endl;
        out << "Diversity Controls:            MMAS bounds, Adaptive restarts" << std::endl;
    ...
    if (writeFile) {
        ...
        // Show every 100th iteration, plus first and last
...
void runExperimentMulti(const InstanceConfig& config, int runs, int runNumberForSummary = 1) {
    ...
    if (instance_size > 1000) {
        // Use 4 threads for very large instances to balance performance vs memory contention
        num_threads = 4;
    } else if (instance_size > 500) {
        // Use 6 threads for large instances
        num_threads = 6;
    } else {
        // Use up to 8 threads for smaller instances
        num_threads = std::min(8, static_cast<int>(std::thread::hardware_concurrency()));
    }
    ...
    std::string summaryFile = "results/ACO_opt_" + config.name + "_run" + std::to_string(runNumberForSummary) + ".txt";
...
void runAllInstancesSequentially(const std::vector<InstanceConfig>& instances, int runs) {
    ...
    std::cout << "\nAll OPTIMIZED ACO experiments completed!" << std::endl;
}

int main(int argc, char* argv[]) {
    // Focus on the problematic pr1002 instance first
